...
| 29 | Admin Panel | **complete** (2026-05-19) — 6 plans, separate admin/ Vite+React19+TS app in monorepo; Tailwind CSS v3 dark mode + shadcn/ui CSS var tokens; React Router v6 (/login + / + /users + /users/:id); ProtectedRoute + AppShell (top nav + sidebar); sessionStorage auth helpers (getToken/setToken/clearToken/isAuthenticated); Axios Bearer interceptor + 401→logout; TanStack Query v5 (staleTime 30s); TanStack Table v8 server-side pagination + search; React Hook Form + Zod on grant forms; bcryptjs + User.banned Boolean @default(false) Prisma migration; @fastify/jwt + @fastify/cors; 6 admin routes (POST /admin/login bcrypt+JWT 24h / requireAdmin middleware / GET /admin/users paginated search / GET /admin/users/:id / POST grant gold|essence|serum / POST ban); DashboardPage Recharts LineChart (mock 7-day data — real analytics deferred Phase 30); SMOKE_TEST_29.md 9 scenarios A-I; deferred: multi-admin, full analytics, i18n, quest stats, audit log, CSV export, 2FA. 29/29 REQ-IDs covered (PHASE29-ADMIN-APP-SCAFFOLD, PHASE29-VITE-CONFIG, PHASE29-TAILWIND-CONFIG, PHASE29-SHADCN-INIT, PHASE29-ROUTER-SETUP, PHASE29-AXIOS-INTERCEPTOR, PHASE29-REACT-QUERY-SETUP, PHASE29-PRISMA-BANNED-FIELD, PHASE29-FASTIFY-JWT-PLUGIN, PHASE29-FASTIFY-CORS-PLUGIN, PHASE29-BCRYPT-DEP, PHASE29-ADMIN-LOGIN-ROUTE, PHASE29-JWT-MIDDLEWARE, PHASE29-ADMIN-USERS-LIST-ROUTE, PHASE29-ADMIN-USER-DETAIL-ROUTE, PHASE29-ADMIN-GRANT-ROUTE, PHASE29-ADMIN-BAN-ROUTE, PHASE29-LOGIN-SCREEN, PHASE29-DASHBOARD-PLACEHOLDER, PHASE29-USERS-LIST-TABLE, PHASE29-USER-DETAIL-VIEW, PHASE29-GRANT-FORMS, PHASE29-BAN-TOGGLE, PHASE29-APP-SHELL, PHASE29-PROTECTED-ROUTE, PHASE29-ENV-CONFIG, PHASE29-BUILD-CHAIN, PHASE29-SMOKE, PHASE29-FINALIZE). |
| 28 | Quests | **complete** (2026-05-19) — 6 plans, ~36h scope (5min + 9min + 10min + 8min + 8min + 45min per-plan; 5 atomic feat/refactor commits per plan + per-plan SUMMARYs); 8-я tab «Квесты» 📜 в Cosmic Hub (cosmos-gated, наследует Phase 22-06 modal-level gate) с активной очередью (cap 5) + collapsible completed history (MAX_VISIBLE=20, persistence cap 100); 40 QuestConfig entries в config/quests.ts (covers 100% raceChains.ts quest_hook stubs — actual=40 vs phase-outline=60 documented в 28-02 SUMMARY); 4 quest types (📦 Доставка / 🔍 Разведка / ⚡ Мерж / 🤝 Дипломатия — distribution 8/14/11/7) × 7 QuestTarget shapes (serum_count/gold_amount/planets_visited/missions_complete/merge_to_level/merge_count/raise_relationship); 4 reward kinds (essence 13 / gold 11 / serum 10 / relationship_and_bonus 6 с opaque bonus_id tokens — gold_income_1pct/ship_speed_1pct/serum_drop_1pct/shop_discount_1pct); 20 easy + 10 medium (_b) + 10 hard (_c) difficulty distribution; auto-activation от Phase 27 «Поддержать» на quest_hook ChainItem (slice resolveAccept extended; cap-hit retains +1 relationship); auto-complete при progress reaches target → reward popup (QuestRewardPopup createPortal к document.body, z-index 199 backdrop / 200 content peer FirstContactModal, CSS @keyframe quest-reward-slide-in 250ms ease-out scale 0.85→1, dedup-by-activeQuestId queue cap 10 с DEV-warn overflow); auto-dismiss 5s + Escape + backdrop + CTA «Забрать»; manual cancel inline confirm panel внутри QuestCard (вместо отдельного modal — simpler overlay stack) → -1 relationship penalty к race-owner'у через applyDeltaClamp shared с pendingEngine + 'quests:cancelled' + 'contacts:relationship-delta' events; progress hooks subscribed: merge:happened (merge_count + merge_to_level через discoveredLevels), cosmic:box-opened (serum_count by element), starmap:planet-select (planets_visited), cosmic:ship-arrived (missions_complete), contacts:relationship-delta (raise_relationship); polling-driven targets (gold_amount, raise_relationship) reconcile через reconcileQuestProgress() — QuestsTab mount effect + boot-time call в QuestController; pure questEngineTick deterministic side-effect-free (5 functions: activateQuestFromHook + checkActiveQuestsProgress + applyQuestReward + isQuestComplete + generateActiveQuestUuid) + 24 vitest unit tests с vi.mock'd QUESTS fixture (independent от Plan 28-02 data fill — mirror Phase 27-03 pendingEngine pattern); persistence + server sync через cosmic blob (snapshotForSave + loadGameState hydrate + defensive load strip unknown questId + FIFO trim completedQuests at 100 newest; gameSync.test.ts coverage REQUIRED_COSMIC_SYNC_FIELDS extended 2 PASS regression intact); 4 new typed eventBus events (quests:activated + quests:cap-reached + quests:completed с full QuestReward payload + quests:cancelled); 5 DEV helpers __activateQuest/__progressQuest/__completeQuest/__resetQuests/__dumpQuests (DEV-gated через import.meta.env.DEV — Vite tree-shake confirmed 0 hits for all 5 в dist/assets/index-*.js); RelationshipBar pulse inherited Phase 27 cancel penalty path; cliclability checklist (type=button + touchAction:manipulation + stopPropagation + z-index hierarchy QuestRewardPopup 199/200 / Cosmic Hub 100); no Lottie (CSS keyframes + Phaser tweens only, memory feedback_animations); НЕ trogает frog.container.alpha (DOM-only); tab strip 8 buttons fit на 320px viewport — Plan 28-04 deferred к Plan 28-06 SMOKE с enumerated fallback options (reduce padding `12px 4px → 12px 2px` / icon-only @media / scroll-strip); SMOKE_TEST_28.md 120 строк, 6 scenarios A-F (tab visibility + cosmos gate / activation from Contacts / progress per type / completion + reward popup / cancel + penalty / persistence + cap) + i18n PARITY + BUILD CHAIN + REGRESSION SANITY; bundle delta gzip main **+21.73 KB** (Phase 27 baseline 220.94 KB → **242.67 KB**; target ~+15 KB exceeded — substantive feature surface: quest engine + 40 configs + UI + reward popup + 111 i18n leaves), CosmicHubModal chunk +1.23 KB (15.61 → **16.84 KB**); vitest 117 → **198 PASS** / 0 FAIL / 1 skip (+81 tests: 24 questEngine + 57 misc); i18n RU/EN/ES parity 522 → **633 keys × 3 locales = 1899 entries** (+111 per locale: 14 cosmic_hub.quests.* + 80 quests.<id>.{description,short} + 17 misc placeholder/reward/cancel keys). 28/28 ✓ REQ-IDs (PHASE28-QUEST-CONFIG, PHASE28-QUEST-DATA-60-MAPPINGS, PHASE28-QUEST-TYPES-4, PHASE28-ACTIVE-QUESTS-STATE, PHASE28-COMPLETED-QUESTS-STATE, PHASE28-CAP-5, PHASE28-AUTO-ACTIVATE-FROM-HOOK, PHASE28-AUTO-COMPLETE-PROGRESS, PHASE28-MANUAL-CANCEL-PENALTY, PHASE28-PROGRESS-HOOKS-EVENTBUS, PHASE28-REWARD-ESSENCE, PHASE28-REWARD-SERUM, PHASE28-REWARD-GOLD, PHASE28-REWARD-DIPLOMACY, PHASE28-QUESTS-TAB-UI, PHASE28-QUEST-CARD, PHASE28-REWARD-POPUP, PHASE28-PERSISTENCE, PHASE28-SERVER-SYNC, PHASE28-I18N-RU, PHASE28-I18N-EN, PHASE28-I18N-ES, PHASE28-I18N-PARITY, PHASE28-COSMOS-GATE, PHASE28-DEV-HELPERS, PHASE28-CLICLABILITY, PHASE28-SMOKE, PHASE28-FINALIZE). |
| 27 | Contacts + Messages + Relationships foundation | **complete** (2026-05-18) — 6 plans, ~22h scope; 7-я tab «Контакты» 📡 в Cosmic Hub (cosmos-gated, наследует Phase 22-06 modal-level gate) с list view 10 рас + race detail (back arrow + lore + RelationshipBar + pending interaction); RACE_CHAINS 10 рас × 10 ChainItem (40 msg + 30 dialog + 20 quest_hook + 10 event); pure pendingEngineTick (lowest-progress-first pull, cap=3, event auto-apply, cosmos + firstContact gate) + 13 vitest; cosmic slice +3 fields (raceRelationships INITIAL=2 / chainProgress / pendingItems) + 4 actions (resolveAccept/Refuse/Acknowledge/triggerPendingPull); 2 typed eventBus events (contacts:relationship-delta + contacts:event-applied); EventToast top-center stack (max 3, auto-dismiss 3s, CSS keyframes, z-index 150); persistence + server sync через cosmic blob (defensive load clamps relationship to [1,10]); DEV helpers __addPending/__resetRelationships/__advanceChain/__dumpContacts; RelationshipBar pulse на tier change; quest hooks STUB для Phase 28 (cosmic_hub.contacts.quest_stub hint); SMOKE_TEST_27.md 140 строк, 6 scenarios A-F; bundle delta gzip main +11.77 KB (Phase 26 baseline 209.17 KB → 220.94 KB; cap ~+15 KB ✓), CosmicHubModal chunk +1.35 KB (14.26 → 15.61 KB); i18n RU/EN/ES parity 402 → **522 keys × 3 locales = 1566 entries** (+120 keys per locale: 15 cosmic_hub.contacts.* + 100 races.<id>.chain.<step>.* (10 races × 10 steps) + 5 cosmos.event.* + 1 notification template); никакого Lottie (CSS keyframes для tier pulse + toast slide/fade); НЕ trogает frog.container.alpha (DOM-only); cliclability checklist (type='button' + touchAction: manipulation + stopPropagation). 25/25 ✓ REQ-IDs (PHASE27-CONTACTS-TAB, PHASE27-CONTACTS-LIST, PHASE27-RACE-DETAIL, PHASE27-RELATIONSHIP-STATE, PHASE27-RELATIONSHIP-TIERS, PHASE27-CHAIN-CONFIG, PHASE27-CHAIN-DATA-10-RACES, PHASE27-PENDING-ENGINE, PHASE27-PENDING-CAP-3, PHASE27-REPLY-UX, PHASE27-EVENT-INLINE, PHASE27-TOAST-SYSTEM, PHASE27-QUEST-HOOK-STUB, PHASE27-PERSISTENCE, PHASE27-SERVER-SYNC, PHASE27-I18N-RU/EN/ES/PARITY, PHASE27-COSMOS-GATE, PHASE27-FIRST-CONTACT-DEP, PHASE27-DEV-HELPERS, PHASE27-CLICLABILITY, PHASE27-SMOKE, PHASE27-FINALIZE). |
