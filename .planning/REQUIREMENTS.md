...
## Phase 30 — Deploy cut (2026-06-11)

| ID | Requirement |
|----|-------------|
| CUT30-REMOVE-FACTORY | Полностью удалить систему заводов: все 3 зоны/локация (логика, контроллеры — напр. FactoryBoxController, рендер, спавн, состояние). Никаких заводских сущностей в рантайме. |
| CUT30-REMOVE-DRONES | Удалить всю логику дронов: заряд, RTB-маршрут, модалки (DronerModal и связанные), multi-drone, любые дрон-таймеры/тики. |
| CUT30-EMPTY-ZONES | Зоны заводов рендерятся пустыми. Мульти-локационная структура локаций НЕ удаляется. |
| CUT30-UNIFY-MERGE | Merge/work-логика, отличающаяся по локациям, сведена к единому общему поведению. Конец per-location ветвлениям. |
| CUT30-SINGLE-UPGRADE | Прогрессия откатана к одной прокачке (до-локационный upgrade-экран). Точная цель определена research'ем по git-истории/архив-ветке. |
| CUT30-STATE-CLEANUP | Удалить мёртвое состояние/persistence/store-поля, завязанные на заводы/дроны. Миграция безопасна (no data-loss crash на старых сейвах). |
| CUT30-NO-ORPHANS | Нет orphan-импортов/ссылок/мёртвых i18n-ключей. `tsc` + build + тесты зелёные. |
| CUT30-SMOKE | Smoke-чеклист cut-сборки: запуск, локации переключаются, прокачка работает, нет ошибок консоли по заводам/дронам. |

---

**Last updated:** 2026-06-11 — Phase 30 deploy-cut requirements (8 CUT30-* IDs)
