 ...
 */
export function deriveModulations(seed: number, _archetype: string): PlanetModulations {
 ...
 */
export function cutoffHz(bin: number, range: [number, number] = [400, 4000]): number {
  const [lo, hi] = range
...
const C = 60, D = 62, Eb = 63, E = 64, F = 65, Fs = 66, G = 67, A = 69, Bb = 70, B = 71

const PROFILES: Record<string, VoiceProfile> = {
  // ── Main race types ──
  home:        { notes: [C, E, G, C + 12], synths: ['bell', 'drone'] },
  crystal:     { notes: [E + 12, G + 12, B + 12], synths: ['bell', 'pluck'], detune: 5 },
  rocky:       { notes: [C - 24, G - 12], synths: ['membrane', 'drone'] },
  ancient:     { notes: [A - 12, E, A], synths: ['drone', 'bell'], octaveOffset: -12 },
  ...
  energy:      { notes: [E + 12, G + 12, B + 12, E + 24], synths: ['fm', 'pluck'], detune: 8 },
  mist:        { notes: [D, A, D + 12], synths: ['drone', 'noise'], noiseType: 'pink' },
  aquatic:     { notes: [G, B, D + 12], synths: ['bell', 'noise'], noiseType: 'pink' },
  shadow:      { notes: [C - 24, Eb - 12], synths: ['drone', 'noise'], noiseType: 'brown' },
  aerial:      { notes: [G + 12, B + 12, D + 24], synths: ['pluck', 'noise'], noiseType: 'white' },

  // ── BG archetypes ──
  gas_giant:   { notes: [C - 12, G - 12, C], synths: ['drone', 'fm'] },
  gas_ringed:  { notes: [C - 12, G - 12, E + 12, G + 12], synths: ['drone', 'bell'] },
  ice:         { notes: [E + 12, B + 12, E + 24], synths: ['bell', 'pluck'] },
  ocean:       { notes: [F, A, C + 12], synths: ['drone', 'noise'], noiseType: 'pink' },
  desert:      { notes: [F, C + 12], synths: ['noise', 'drone'], noiseType: 'pink' },
  lava:        { notes: [C - 12, Eb - 12], synths: ['membrane', 'noise', 'fm'], noiseType: 'brown' },
  forest:      { notes: [G, B, D + 12], synths: ['pluck', 'noise'], noiseType: 'pink' },
  mineral:     { notes: [C + 12, G + 12], synths: ['bell', 'membrane'] },
  dead:        { notes: [C - 24], synths: ['drone'] },
  toxic:       { notes: [Eb, A], synths: ['drone', 'fm'], detune: 20 },
  plasma:      { notes: [G + 12, D + 24, A + 24], synths: ['fm', 'pluck'], detune: 15 },
  binary:      { notes: [C, F], synths: ['drone', 'fm'] },
}

function Cs(): number { return C + 1 } // C# (использован в mystic)
...
class PlanetVoice {
  ...
  private async ensureReady(): Promise<void> {
    ...
    this.loadingPromise = (async () => {
      ...
      this.kit = { bus, bell, drone, pluck, noise, fm, membrane, droneFilter, noiseFilter }
    })()
  ...
  play(typeOrArchetype: string, durationMs: number, archetypeFallback?: string, seed?: number): void {
    ...
    const profile = PROFILES[typeOrArchetype] ?? (archetypeFallback ? PROFILES[archetypeFallback] : null)
    if (!profile) return
