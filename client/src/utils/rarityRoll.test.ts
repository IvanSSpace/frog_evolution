...
// ─── Test 1: distribution within ±5% from 50/35/12/3 ───
{
  ...
  assert(
    Math.abs(cf - 0.5) < 0.05,
    `Test 1: common dist ${cf} not within ±5% of 0.5`,
  )
  assert(
    Math.abs(rf - 0.35) < 0.05,
    `Test 1: rare dist ${rf} not within ±5% of 0.35`,
  )
  assert(
    Math.abs(ef - 0.12) < 0.05,
    `Test 1: epic dist ${ef} not within ±5% of 0.12`,
  )
  assert(
    Math.abs(lf - 0.03) < 0.04,
    `Test 1: legendary dist ${lf} not within ±4% of 0.03`,
  )
}
...
for (let i = 0; i < 200; i++) {
  ...
  assert.equal(
    r,
    'legendary',
    `Test 2: expected legendary at pity 25, got ${r}`,
  )
}
...
for (let i = 0; i < 200; i++) {
  ...
  assert(
    r === 'epic' || r === 'legendary',
    `Test 3: expected epic/legendary at pity.epic 10, got ${r}`,
  )
}
...
// ─── Test 7-9: updatePity behavior ───
{
  ...
  assert.deepEqual(
    ...
    'Test 7: common increments all',
  )

  const u2 = updatePity({ rare: 5, epic: 5, legendary: 5 }, 'rare')
  assert.deepEqual(
    u2,
    { rare: 0, epic: 6, legendary: 6 },
    'Test 8: rare resets rare, increments epic/legendary',
  )
...
// ─── Test 10: epic resets rare/epic, increments legendary ───
{
  ...
  assert.deepEqual(
    u,
    { rare: 0, epic: 0, legendary: 6 },
    'Test 10: epic resets rare/epic',
  )
}
