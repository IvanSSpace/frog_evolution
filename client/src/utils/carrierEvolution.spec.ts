...
import {
  TIER_RANGES, TIER_BUCKET_WEIGHTS, ceilingForBucket, bucketOfCeiling,
  rollBucket, shouldForceS, rollCeilingForCarrier, rollFeedOutcome,
  SUCCESS_RATE_BASE, type Bucket,
...
export function run(): void {
  // TIER_RANGES
  assert(TIER_RANGES.common.min === 1 && TIER_RANGES.common.max === 6, 'TIER_RANGES.common')
  assert(TIER_RANGES.rare.min === 7 && TIER_RANGES.rare.max === 12, 'TIER_RANGES.rare')
  assert(TIER_RANGES.epic.min === 13 && TIER_RANGES.epic.max === 18, 'TIER_RANGES.epic')
  assert(TIER_RANGES.legendary.min === 19 && TIER_RANGES.legendary.max === 24, 'TIER_RANGES.legendary')

  // weights sum to 100
  const sumW = TIER_BUCKET_WEIGHTS.S + TIER_BUCKET_WEIGHTS.A + TIER_BUCKET_WEIGHTS.B + TIER_BUCKET_WEIGHTS.C
  ...
  const buckets: Bucket[] = [
    ...
    rollBucket(() => 0.10),  // A (r=10 < 20)
    rollBucket(() => 0.30),  // B (r=30 < 50)
    rollBucket(() => 0.99),  // C
  ]
  assert(buckets[0] === 'S' && buckets[1] === 'S' && buckets[2] === 'A' &&
         buckets[3] === 'B' && buckets[4] === 'C', 'rollBucket boundaries')
  ...
  const carriers3C = [c(2, 2, true, 1000), c(3, 3, true, 2000), c(1, 1, true, 3000)]
  ...
  const carriers2C1A = [c(2, 2, true, 1000), c(5, 5, true, 2000), c(3, 3, true, 3000)]
  ...
  assert(r1.bucket === 'S' && r1.ceiling === 6 && r1.forced === false, 'rollCeiling empty rng0 → S6')

  // rollCeilingForCarrier — forced via streak
  const r2 = rollCeilingForCarrier('common', carriers3C, () => 0.99)
  assert(r2.forced === true && r2.bucket === 'S' && r2.ceiling === 6, 'forced overrides rng')

  // rollFeedOutcome
  const o1 = rollFeedOutcome({ level: 5, ceiling: 6, stabilized: false }, () => 0)
  assert(o1.result === 'success' && o1.newLevel === 6 && o1.newStabilized === false, 'success rng0')
  const o2 = rollFeedOutcome({ level: 6, ceiling: 6, stabilized: false }, () => 0)
  assert(o2.result === 'stabilize' && o2.newLevel === 6 && o2.newStabilized === true, 'stabilize at ceiling')
  const o3 = rollFeedOutcome({ level: 5, ceiling: 6, stabilized: true }, () => 0)
  assert(o3.result === 'stabilize' && o3.newStabilized === true, 'pre-stabilized → stabilize idempotent')
  const o4 = rollFeedOutcome({ level: 5, ceiling: 6, stabilized: false }, () => 0.99)
